		...
		<form action="insertMember" method="post">
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" id="id"
					placeholder="ID 입력(5-10)" name="id" maxlength="10"
					required="required" pattern=".{5, 10}">
...
//-->
	...
	$(function() {
		$('#btn1').click(function() {
			let param = $('#id').val();
			$.ajax({
				url: "checkId?id="+param,
				success: function(data) {
					console.log(data);
					if(data == 1) {
						$('#target').css('background-color', 'red').css('color', 'white').html('사용 중인 아이디입니다.');
					} else {
						$('#target').css('background-color', 'blue').css('color', 'white').html('사용 가능한 아이디입니다.');
					}
				}
			});
		});
